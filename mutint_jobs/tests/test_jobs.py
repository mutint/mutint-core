...
immediate one the rest of the suite runs under, for the reason `mutint_import/tests/test_tasks.py`
...
class AttributionTestCase(TestCase):
    ...
    def test_enqueueing_records_who_asked(self):
        ...
        self.assertEqual(job.task_path, "mutint_import.tasks.build_coverage")
    ...
    def test_an_unknown_result_is_not_an_error(self):
        ...
        self.assertEqual(queue.STATUS_UNKNOWN,
                         queue.status_of("00000000-0000-0000-0000-000000000000",
                                         "mutint_import.tasks.build_coverage"))
...
class CancellationFlagIsSafeToAskTestCase(TestCase):
    ...
    A dev database predating this app has no `mutint_jobs_job` table, and `build_coverage` now
