...
directly, and is the only function in mutint-core that knows which backend is configured. It is
...
logger = logging.getLogger("mutint_jobs.queue")
...
# The two statuses that mean the work is over. `mutint_jobs.views` has its own copy for a
...
def uses_database_backend():
    ...
    Public because `./mutint start` asks it before spawning a worker: under `ImmediateBackend`
    there is nothing to drain and `db_worker` would die on its own `valid_backend_name`. Asked
    here rather than there so this stays the only module in mutint-core that knows the
...
def reap_stranded_rows(cutoff, dry_run=False):
    ...
    **Nothing else clears these.** `django_tasks_db` ships `prune_db_task_results`, and it
    ...
    - **RUNNING past the cutoff**, by `started_at`. These have a real source now that
      `./mutint start` runs a worker: shutdown kills the worker's group after a few seconds
