...
logger = logging.getLogger("aledb_import.vcf_import")
...
def _infer_mob(record, variant, loaded):
    ...
    Off by default (`ALEDB_VCF_INFER_MOB`), and the default is the honest one: **a wrong MOB
    ...
    if record["type"] != "INS" or loaded is None:
        return record
    if not getattr(settings, "ALEDB_VCF_INFER_MOB", False):
        return record
...
def _annotated_repeats(loaded, seq_id):
    ...
    """
    from aledb_import.annotate.model import trim_repeat_name
...
def import_sample(document, sample_name, context, experiment):
    ...
    """
    from aledb_import.gd_import import import_document_as_sample
