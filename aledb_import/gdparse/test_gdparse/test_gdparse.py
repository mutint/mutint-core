...
class TestGDParser(unittest.TestCase):
    ...
    def test_parsing_HTML_with_quotations(self):
        with open(self.path+"no_HTML_in_DB_test/annotated.gd") as output_genomic_diff_file:
    ...
    def test_del_position_522526_gene_name_is_clr(self):
        ...
        mutation_id = 1
        gd_file_name = self.path + "annotated.gd"
    ...
    def test_parsing_breseq_version(self):
        expected = "0.27.0b"
        with open(self.path + "has_AUTHOR.gd") as annotated_gd_file:
        ...
        with open(self.path + "has_PROGRAM.gd") as annotated_gd_file:
            gd_parser = gdparse.GDParser(annotated_gd_file)
        returned_version = gd_parser.meta_data.get(gdparse.BRESEQ_VERSION_KEY)
        self.assertEqual(expected, returned_version)
        expected = None
        with open(self.path + "no_PROGRAM_or_AUTHOR.gd") as annotated_gd_file:
            gd_parser = gdparse.GDParser(annotated_gd_file)
        returned_version = gd_parser.meta_data.get(gdparse.BRESEQ_VERSION_KEY)
        self.assertEqual(expected, returned_version)
        with open(self.path + "AUTHOR_name.gd") as annotated_gd_file:
            gd_parser = gdparse.GDParser(annotated_gd_file)
        returned_version = gd_parser.meta_data.get(gdparse.BRESEQ_VERSION_KEY)
        self.assertEqual(expected, returned_version)
