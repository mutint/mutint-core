...
from aledb_import.upload_session import UploadError, reap_expired_sessions

# An installed app label that is not aledb_import, standing in for a plugin. Any first-party
# app would do; what matters is that `apps.is_installed` says yes.
CONSUMER = "aledb_stats"
...
class StagingTestCase(TestCase):
    ...
    def test_session_for_refuses_another_components_session(self):
        ...
        found, error = staging.session_for(_Request(), session.id, "aledb_export")
