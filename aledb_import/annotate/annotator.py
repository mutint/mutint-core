...
(reference_sequence.cpp:3109-4016) so that ALEdb can annotate a plain breseq
`output.gd` from a reference GenBank file instead of requiring the output of
`gdtools ANNOTATE`.

Mutations are plain dicts, as produced by aledb_import.gdparse -- the same shape as
...
def annotate_mutations(mutations, references, ignore_pseudogenes=False,
    ...
    are not annotated -- ALEdb does not store them per-mutation.
    """
    annotated = []
