...
them existed twice, in `mutint_sample.models.Sample` and in `mutint_experiment.samples`, kept in
...
def format_coordinate(population, time_point, sample):
    ...
    `mutint_experiment.0008`. The time point goes through `format_time_point`, which is the
...
#:
#: Only ever used to *recognize* one, by `./mutint relabel_samples`. No import has written a
