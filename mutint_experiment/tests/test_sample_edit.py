...
class SampleEditTestCase(TestCase):
    ...
    def make_sample(self, population, time_point, label, source_name="", experiment=None,
                    is_mixed=False):
        experiment = experiment or self.experiment
        population_row, _ = Population.objects.get_or_create(
            experiment=experiment, name=population)
        return Sample.objects.create(
            population=population_row, time_point=time_point, name=label,
            is_clonal=not is_mixed, source_name=source_name)

    def row(self, sample, **overrides):
    ...
    def single(self, sample, **overrides):
    ...
    def coordinate(self, sample):
        ...
        return sample_coordinate(sample)
...
class SampleEditPagesTestCase(SampleEditTestCase):
    def setUp(self):
        super().setUp()
        self.sample = self.make_sample(1, 1, "1-1", source_name="first")
    ...
    def test_the_sample_page_renders(self):
        ...
        for field in ('id="se-name"', 'id="se-population"', 'id="se-time-point"',
                      'id="se-sample"', 'id="se-save"'):
    ...
    def test_the_bulk_page_lists_this_experiment_only(self):
        ...
        stranger_sample = self.make_sample(1, 1, "1-1", source_name="elsewhere", experiment=other)
...
class DescriptiveEditTestCase(SampleEditTestCase):
    ...
    def test_renaming_a_sample_saves(self):
        response = self.single(self.sample, source_name="renamed")
    ...
    def test_a_bulk_save_leaves_the_other_groups_alone(self):
        ...
        self.assertEqual(200, self.bulk(
            [self.row(self.sample, population=3, medium_description="LB")]).status_code)

        self.sample.refresh_from_db()
        self.assertEqual("LB", self.sample.curation["medium_description"])
        self.assertEqual(68.0, self.sample.breseq["mean_coverage"])

    def test_the_description_saves(self):
        self.single(self.sample, description="colony from day 30")
    ...
    def test_a_duplicate_sample_name_is_refused(self):
        ...
        self.make_sample(1, 1, "2-1", source_name="second")

        response = self.single(self.sample, source_name="second")
    ...
    def test_an_over_long_value_is_refused_rather_than_truncated(self):
        response = self.single(self.sample, source_name="x" * 201)
...
class RenumberTestCase(SampleEditTestCase):
    def setUp(self):
        super().setUp()
        self.first = self.make_sample(1, 1, "1-1", source_name="first")
        self.second = self.make_sample(1, 1, "2-1", source_name="second")
    ...
    def test_renumbering_one_sample_leaves_its_sibling_alone(self):
        ...
        self.assertEqual(200, self.single(self.first, population=2).status_code)
    ...
    def test_the_sample_keeps_its_primary_key(self):
        ...
        self.assertEqual(200, self.single(self.first, population=7, time_point=9).status_code)
    ...
    def test_a_fresh_coordinate_creates_the_rows_it_needs(self):
        ...
        before = self.chain_counts()
        self.single(self.first, population=5, time_point=5, name="5-5")
        population_names, flasks, samples = self.chain_counts()

        self.assertEqual(before[0] + 1, population_names)
    ...
    def test_moving_onto_a_free_label_beside_an_occupied_one_works(self):
        ...
        two rows at one (flask, sample_label) existed in real data and `get_or_create`
        ...
        response = self.single(self.first, name="2-4")
    ...
    def test_a_named_ale_is_accepted(self):
        ...
        self.assertEqual(200, self.single(self.first, population="Ara+1").status_code)
    ...
    def test_and_so_is_a_named_isolate(self):
        ...
        self.assertEqual(200, self.single(self.first, name="763A").status_code)
    ...
    def test_a_non_integer_time_point_is_still_refused(self):
        ...
        response = self.single(self.first, time_point="five hundred")
        self.assertEqual(400, response.status_code)
    ...
    def test_a_blank_ale_is_refused(self):
        ...
        self.assertEqual(400, self.single(self.first, population="  ").status_code)
        self.assertEqual(("1", 1, "1-1"), self.coordinate(self.first))

    def test_an_over_long_label_is_refused_rather_than_truncated(self):
        response = self.single(self.first, name="x" * 101)
        self.assertEqual(400, response.status_code)
    ...
    def test_a_negative_number_is_refused(self):
        self.assertEqual(400, self.single(self.first, time_point=-1).status_code)

    def test_ale_zero_is_allowed(self):
        """A population named "0" is legal: it is where an ancestor was filed."""
        self.assertEqual(200, self.single(self.first, population=0).status_code)
        self.assertEqual(("0", 1, "1-1"), self.coordinate(self.first))

    def test_moving_onto_an_occupied_coordinate_is_refused(self):
        response = self.single(self.first, name="2-1")
    ...
    def test_a_non_owner_cannot_edit(self):
        stranger = User.objects.create(
            username="stranger", email="s@e.com", is_active=True, is_staff=True)
        self.client.force_login(stranger)
        self.assertEqual(403, self.single(self.first, population=2).status_code)


class OrphanPruningTestCase(SampleEditTestCase):
    def setUp(self):
        super().setUp()
        self.only = self.make_sample(1, 1, "1-1", source_name="only")
    ...
    def test_moving_the_last_sample_out_removes_the_rows_it_emptied(self):
        ...
        self.single(self.only, population=2, time_point=2, name="2-1")
    ...
    def test_a_population_that_still_holds_another_sample_survives(self):
        self.make_sample(1, 1, "2-1", source_name="sibling")

        self.single(self.only, name="5-1")
    ...
    def test_an_emptied_time_point_leaves_nothing_to_prune(self):
        ...
        self.make_sample(1, 9, "1-1", source_name="alone")

        self.single(self.only, time_point=9, name="7-1")
...
class BulkSampleEditTestCase(SampleEditTestCase):
    def setUp(self):
        super().setUp()
        self.first = self.make_sample(1, 1, "1-1", source_name="first")
        self.second = self.make_sample(1, 2, "1-1", source_name="second")
    ...
    def test_a_swap_succeeds_in_one_save(self):
        ...
        response = self.bulk([self.row(self.first, time_point=2),
                              self.row(self.second, time_point=1)])
    ...
    def test_one_bad_row_saves_nothing(self):
        response = self.bulk([self.row(self.first, source_name="renamed"),
                              self.row(self.second, time_point="not a number")])
    ...
    def test_two_rows_claiming_one_coordinate_are_refused(self):
        response = self.bulk([self.row(self.first, time_point=3),
                              self.row(self.second, time_point=3)])
    ...
    def test_a_sample_from_another_experiment_is_refused(self):
        ...
        foreign = self.make_sample(1, 1, "1-1", source_name="foreign", experiment=other)

        response = self.bulk([self.row(foreign, time_point=8)])
    ...
    def test_a_non_owner_cannot_save(self):
        stranger = User.objects.create(
            username="stranger", email="s@e.com", is_active=True, is_staff=True)
        self.client.force_login(stranger)
        self.assertEqual(403, self.bulk([self.row(self.first, time_point=9)]).status_code)
...
class RebuildHooksTestCase(SampleEditTestCase):
    ...
    def test_a_structural_save_rebuilds_once_for_the_whole_batch(self):
        ...
        with run as run_rebuilds, request as request_rebuild:
            response = self.bulk([self.row(self.first, population=2),
                                  self.row(self.second, population=3)])
    ...
    def test_a_structural_save_leaves_the_mutation_counts_alone(self):
        ...
        run, request = self._patched()
        with run as run_rebuilds, request as request_rebuild:
            self.bulk([self.row(self.first, population=2)])
    ...
    def test_a_descriptive_save_rebuilds_nothing(self):
        run, request = self._patched()
        with run as run_rebuilds, request as request_rebuild:
            self.assertEqual(200, self.single(self.first, source_name="renamed").status_code)
...
class ExistingDuplicateNamesTestCase(SampleEditTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.first = self.make_sample(1, 1, "1-1", source_name="dup")
        self.second = self.make_sample(1, 1, "2-1", source_name="dup")

    def test_an_experiment_with_duplicate_names_can_still_be_saved(self):
        response = self.bulk([self.row(self.first, time_point=4),
    ...
    def test_one_of_them_can_be_renamed_to_something_free(self):
        response = self.single(self.first, source_name="distinct")
...
class TimePointLabelingTestCase(SampleEditTestCase):
    ...
    `time_point` and then `TimePoint.value`; the label never changed, which is what
    eventually made the column follow it.
    """

    def setUp(self):
        super().setUp()
        self.sample = self.make_sample(1, 30000, "1-1", source_name="first")
    ...
    def test_the_time_point_input_has_no_stepper(self):
        ...
        field = [line for line in html.splitlines() if 'id="se-time-point"' in line][0]
        self.assertIn('type="text"', field)
        self.assertNotIn('type="number"', field)

    def test_a_refusal_does_not_leak_the_column_name(self):
        response = self.single(self.sample, time_point="not a number")

        self.assertEqual(400, response.status_code)
        message = response.json()["error"]
        self.assertIn("time point", message)
        self.assertNotIn("time_point", message.lower())

    def test_a_fractional_time_point_is_refused_clearly(self):
        """time_point is an IntegerField, so a fraction cannot be stored. The plain
        input invites one, which makes the wording of this refusal load-bearing."""
        response = self.single(self.sample, time_point="30000.5")
    ...
    def test_a_large_time_point_saves(self):
        self.assertEqual(200, self.single(self.sample, time_point=123456).status_code)
