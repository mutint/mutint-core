...
from mutint_experiment.roles import ROLE_CHOICES, ROLE_OWNER
...
class Experiment(SoftDeleteMixin):
    ...
    # `mutint_experiment/ancestor.py` is the whole mechanism.
    #
    ...
    #
    # Named by string because `mutint_experiment` must not import `mutint_sample` at load time;
    ...
    # `mutint_experiment.views.experiment_ancestor_apply` checks it.
    #
    ...
    ancestor = models.ForeignKey("mutint_sample.Sample",
...
class Population(models.Model):
    ...
    **This was `AleId`, and its own docstring already called these populations.** The model
    ...
    `mutint_experiment.ordering.sample_order()` is what every sample listing orders by
