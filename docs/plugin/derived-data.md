...
- **Do not put a foreign key on `Mutation` if you can avoid a hard dependency**, but know what
  you are taking on if you store bare ids instead. `mutint_phylogeny`'s JSON stores `Mutation.id`
...
register_rebuilder('mutint_yourthing', rebuild_your_table)
```
...
rebuild is quiet, which is why `./mutint rebuild --list` exists.
...
leaves the previous answer on screen with the error in `./mutint rebuild --list`, rather than
...
```python
register_rebuilder('mutint_yourthing', rebuild_your_table, auto=False)
```
...
`./mutint rebuild <id> --only mutint_yourthing`, or a button on your own page. `force=True` does
...
recomputing it unasked would be wrong. `mutint-phylogeny` uses it: a tree is an inference with
