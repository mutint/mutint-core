...
aledb-core runs standalone with **no external services** — SQLite, no MySQL, no Docker, no
...
`./aledb` bootstraps a virtualenv at `env/main/` and re-execs under it, so there is nothing to
...
An assembled project collects aledb-core and its plugins as git submodules and discovers them
at startup. Every command below works the same in both.

## Reinstalling

```bash
./aledb install      # or ./mutint install
```
...
./aledb test         # the full suite, ~2 minutes
```
