...
there are two links, **breseq report** and **summary**. They open the report inside ALEdb, with
...
ALEdb. The report is part of the folder, so it arrives with the data.
...
reference — and ALEdb treats any of it as untrusted. The sandbox means the report can draw
itself and run its own code, and cannot reach anything else in ALEdb.
