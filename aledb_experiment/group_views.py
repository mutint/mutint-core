...
render `403.html` with a status, `@require_POST` JSON endpoints posted to by `aledbPost`.
"""
...
from aledb_experiment.models import AleGroup, AleGroupMembership, ProjectAccess
from aledb_experiment.permissions import AccessError, clear_role_cache, resolve_username
...
def groups(request):
    ...
    return render(request, "ale/groups.html", context)
...
def group_new(request):
    ...
    if not request.user.is_authenticated:
        return render(request, "403.html", get_user_context(request.user), status=403)
    return render(request, "ale/group_new.html", get_user_context(request.user))
...
def group_create(request):
    ...
    if AleGroup.objects.filter(name__iexact=name).exists():
        return JsonResponse({"error": 'There is already a group named "%s".' % name},
                            status=400)

    try:
        with transaction.atomic():
            group = AleGroup.objects.create(name=name, description=description,
                                            owner=request.user)
            ...
            AleGroupMembership.objects.create(group=group, user=request.user,
...
def group_detail(request, pk):
    group = get_object_or_404(AleGroup, pk=pk)
    context = get_user_context(request.user)
    ...
    return render(request, "ale/group_detail.html", context)
...
def group_update(request, pk):
    ...
    if AleGroup.objects.filter(name__iexact=name).exclude(pk=group.pk).exists():
        return JsonResponse({"error": 'There is already a group named "%s".' % name},
                            status=400)
...
def group_member_add(request, pk):
    ...
    if AleGroupMembership.objects.filter(group=group, user=user).exists():
    ...
    membership = AleGroupMembership.objects.create(
...
def group_member_update(request, pk):
    ...
    membership = AleGroupMembership.objects.filter(
...
def group_transfer(request, pk):
    ...
    with transaction.atomic():
        ...
        AleGroupMembership.objects.update_or_create(
            group=group, user=user,
            defaults={"is_manager": True, "added_by": request.user})
        AleGroupMembership.objects.filter(
