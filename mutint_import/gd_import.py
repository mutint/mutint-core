...
to a second, breseq-directory-shaped importer; that one is gone and ``./mutint import``
...
stored verbatim in ``Mutation.supplemental_data["mutint_core"]["genome_diff"]`` so it can be
...
logger = logging.getLogger("mutint_import.gd_import")
...
def _next_sample_number(population, time_point):
    ...
    column became text (`mutint_experiment.0008`): `MAX` over strings answers `"9"` for a
...
def _check_seq_ids(document, experiment, sample_name):
    ...
    ``reference_store.known_seq_ids`` for why MutInt does not follow breseq here.
    ...
    """
    from mutint_import.reference_store import known_seq_ids
...
def _database_gd_mutations(seq_experiment, document, experiment=None):
    ...
    unannotated; `./mutint reannotate` fills it in once one arrives."""
...
def _database_uncalled_regions(seq_experiment, document):
    ...
    mutint_stats counts them per sample, and mutint-phylogeny reads them to mark a site
...
def run_post_processing(experiment):
    ...
    in `mutint_common.rebuild_registry` now and this asks for all of it, which is also why
