...
not, because what they protect against is not backend-specific:
    ...
    mutint_import.import_lock   one import at a time, so imports never contend at all
    mutint_import.retry         a sample that loses to some *other* writer tries again
...
`get_or_create` calls -- Experiment, Isolate -- and
`_next_sample_number`'s unlocked read-then-write unreachable by two importers at once.
...
from mutint_import import import_lock, retry
...
class BackendTestCase(TestCase):
    ...
    **The earlier form of this test earned its place immediately**, and the failure it caught
    ...
    five samples -- ran on a different backend configuration from the one mutint-core had
    ...
    def test_it_is_the_database_this_checkout_manages(self):
        ...
        managed = os.environ.get("MUTINT_DB_MANAGED") == "1"
        if not managed:
            self.skipTest("running against an external server")
        self.assertEqual(os.environ["MUTINT_DB_HOST"],
...
class FinalizeUnderLockTestCase(TestCase):
    ...
    def test_the_lock_is_released_when_the_import_fails(self):
        from mutint_import import breseq_folder as folder_module
...
class PollMustNotWriteTestCase(TestCase):
    ...
    def setUp(self):
        ...
        from mutint_experiment.models import Project
        from mutint_experiment.views import _create_experiment
        from mutint_import.models import UploadSession
...
class ImportLockVisibilityTestCase(TransactionTestCase):
    ...
    protecting nothing against the second gunicorn worker or a `./mutint` command. What makes
        ...
        def look():
            ...
            try:
                with connection.cursor() as cursor:
                    cursor.execute(
                        "SELECT holder FROM mutint_import_importlock WHERE name = %s",
