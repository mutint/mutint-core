...
from mutint_import import gd_import, reference_store
...
class GdImportTestCase(TestCase):
    ...
    def test_imported_experiment_pages_render(self):
        ...
        # The per-sample table, not the cross-sample one: Compare is the mutint-compare
...
class UncalledRegionTestCase(TestCase):
    ...
    mutint-phylogeny reads them to mark a site *ambiguous* rather than ancestral.
    """
    ...
    def test_an_unreadable_bound_is_skipped_and_the_import_survives(self):
        ...
        with self.assertLogs("mutint_import.gd_import", level="WARNING"):
...
class SeqIdMustMatchTheReferenceTestCase(TestCase):
    ...
    MutInt holds many experiments side by side and two may be against different
    ...
    def test_a_version_suffix_is_not_trimmed_away(self):
        ...
        MutInt will not quietly treat them as one reference."""
