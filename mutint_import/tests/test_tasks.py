...
(`MutintTestRunner.TASKS`), which is right for every *other* test -- one asserting what an
...
from mutint_import import breseq_folder, coverage, tasks
...
class DatabaseBackendTestCase(TestCase):
    ...
    def test_enqueueing_stores_the_task_and_runs_nothing(self):
        ...
        self.assertEqual("mutint_import.tasks.build_coverage", row.task_path)
...
class CoverageIsAttributedTestCase(TestCase):
    ...
    def test_the_importer_enqueues_through_mutint_jobs(self):
    ...
    def test_a_job_row_carries_the_owner_and_the_sample(self):
        job = jobs.enqueue(tasks.build_coverage, 123456789, user=self.user,
                           label="Coverage — 763A", component="mutint_import",
                           cancellable=True)
