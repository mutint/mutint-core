"""The import types mutint-core itself provides.

Each is registered with ``mutint_common.import_registry`` from ``ImportConfig.ready()``, exactly
...
logger = logging.getLogger("mutint_import.handlers")
...
BRESEQ_PATTERNS = [
    ...
    # the stored reference. See mutint_import.breseq_folder.
    #
...
def breseq_sample_prefixes(staged_root):
    ...
    from mutint_import.breseq_folder import find_sample_dirs
...
def handle_genomediff(experiment, staged_root, paths, user):
    ...
    if total:
        ...
        from mutint_import.gd_import import run_post_processing
...
def list_vcf_units(staged_root, claimed):
    ...
    """
    from mutint_import import vcf, vcf_import
...
def handle_vcf(experiment, staged_root, paths, user):
    """Import variant calls, one MutInt sample per VCF sample column."""
    from mutint_import import vcf, vcf_import
    from mutint_import.gd_import import prepare_experiment_by_id, run_post_processing
    from mutint_import.reference_store import has_reference
