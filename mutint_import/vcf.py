"""Reading VCF, and turning each call into the GenomeDiff record MutInt stores.
...
class VcfLine:
    ...
    here reformats a number or reorders a key, because a VCF exported from MutInt should be the
    VCF that came in.
    """
...
def looks_like_vcf(text):
    ...
    The same rule gdtools applies, and the same rule `mutint_import.sniff` uses for the other
