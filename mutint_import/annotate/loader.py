...
form (`mutint_import.reference.normalize_reference`) is deliberately genes-only
...
def detect_format(path, original_name=None):
    ...
    Named after `mutint_import.reference.detect_format`, but deliberately narrower:
