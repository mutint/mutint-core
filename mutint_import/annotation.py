...
This is the database-aware glue; `mutint_import.annotate` itself knows nothing about
...
one artifact can serve all three; see `mutint_import.reference.normalize_reference`.
...
def reannotate_experiment(experiment, mutations=None, references=None, dry_run=False,
    ...
    Extracted from `./mutint reannotate` so the command and the contig-rename path share one
