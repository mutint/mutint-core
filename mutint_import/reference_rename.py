...
def apply_rename(experiment, reference, plan, actor=""):
    ...
    """
    from mutint_sample.models import Mutation, UncalledRegion
    ...
    with transaction.atomic():
        ...
        counts["evidence"] = _rename_evidence(
            UncalledRegion, experiment, mapping)
...
def _reannotate(experiment):
    ...
    data that `./mutint reannotate` can rebuild, so failing here must not present as the
    ...
    try:
        ...
        logger.exception(
            "re-annotation after renaming experiment %s failed; run `./mutint reannotate %s`",
