...
`reseq_id` is accepted and used only to keep the way back pointing where somebody came from.
...
`ncbi_check` is keyed on `(ale_experiment_id, seq_id)`, not on a mutation: an accession
