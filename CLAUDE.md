...
page, or automatically when the reference was imported from an accession in the first place
(see **An accession can be an input, not only a claim**). The command is for bulk work and for
a deployment being set up from a shell.
...
Migration numbers cited below (`mutint_sample.0010` and the like) predate the migration
collapses of September 2026 and no longer name files; read them as history. The history has
been thrown away and regenerated twice, so **no migration this file names still exists** --
what is left is one initial migration per app (two where a circular foreign key forces the
split), and the data steps are gone with the databases they were written for. Cite the rule a
migration established, never the file.
...
sample would mint a second `Mutation` and split its calls across both. It wrote
...
right. (A reference *downloaded* from an accession is the one case where the answer is known
without asking, and it is still not an inference from a name -- see **An accession can be an
input, not only a claim** below. Nothing about a file path changed.) A pattern strict enough to reject `REL606` would also reject accession formats it was
...
#### An accession can be an input, not only a claim

The section above says a contig's accession is something a person *states* and NCBI then
settles. That stays true of every file path. What it is not true of is a reference that
**arrived from** an accession: `/import/`'s Reference Sequence and Update Annotation tabs take
a box of accessions -- nucleotide (`NC_000913.3`) or genome assembly (`GCF_`/`GCA_`), separated
by commas, spaces or new lines -- and download the records into the drop, alongside whatever
files were dropped with them.

**Downloaded is evidence; typed is a proposal.** `ncbi.record_downloaded` writes VERIFIED with
no call to `verify` and no second download, and that is the rule met at the other end rather
than an exception to it. `verify` exists to fetch NCBI's copy and compare it with what is
stored; here what is stored *is* NCBI's copy, so re-fetching could only agree with itself at
the cost of downloading every imported genome twice. Two things keep it honest, and both are
upstream: `ncbi_fetch._records_in` reads the accession off the **VERSION line of the file that
was actually written** rather than from what was asked for, and
`upload_session._record_downloads` records only digests that reached `seq_ids`. So a row is
written when, and only when, these exact bases were both sent by that record and stored.

**That `seq_ids` intersection is the whole success condition**, which is why no import handler
learned anything about this: a `replace_annotation` against the wrong genome never reaches
`seq_ids`, nor does a failed establish, nor a contig dropped as a duplicate of one already
there. The two core types declare `accepts_accessions=True` on the registry and are otherwise
untouched.

**The files merge because the download lands in staging before `run_import` walks it.**
`ncbi/<typed accession>.gbk`, one file per accession somebody typed, an assembly's member
sequences all in its one file. From there announcement, per-file rows, the all-or-nothing merge
in `_ingest_reference` and the rename confirmation are all the behaviour that already existed.
Its own subdirectory, rebuilt wholesale each time: a declined-then-confirmed rename finalizes
the same session twice, and a second download landing beside the first would import the genome
twice over.

**Resolved at `create_upload_session`, downloaded at finalize**, and the split is the point. A
typo refused at finalize would arrive after the upload and take the staged files with the
failed session; refused at create it costs the round trip that caught it and no session row at
all. It is `mutint-breseq._preflight`'s reasoning at a better moment. The resolve sits **after**
`can_edit_experiment`, or the endpoint is an open relay to NCBI for anybody signed in.

**The download is outside `import_lock`**, which is one-at-a-time for the whole installation:
holding it across somebody's network would stop every other drop. The cost is that two
simultaneous imports can together pass NCBI's keyless 3/second limit; that is answered as a
429 with a sentence, and establishing a reference happens once per experiment, so there is no
cross-process limiter.

Four things found by running it against the real service, each with a test:

- **`rettype` must be `gbwithparts`.** A CON record -- most RefSeq chromosomes -- comes back
  from a plain `gb` fetch with its features, a LOCUS line stating its length and *no ORIGIN
  block*, which Biopython parses into a record whose `.seq` raises `UndefinedSequenceError`.
  The reference would be established with the right contig names and no bases.
- **Datasets paginates.** `sequence_reports` answers a complete genome in one page and a draft
  assembly in several. Stopping at the first would import a *partial* genome, which looks
  exactly like success until a sample is refused against it; `total_count` is cross-checked for
  the same reason.
- **Its API key is a header (`api-key`), where eutils takes a query parameter.** Sending it the
  wrong way is not an error anybody sees -- it silently just does not raise the limit.
- **Two records can share bases**, so the digest-keyed result map collapses them. Arrival is
  counted separately, or the collapsed one reads as a record NCBI never sent. Only one link can
  exist for one sequence anyway (`unique(database, sha256)`); the first typed wins, in
  `_record_downloads` as well.

**`mutint_sample/ncbi.py`'s client bits went public for this** -- `request_params`,
`timeout_seconds`, `max_bases`, `summary`, `Unreachable` -- because the fetcher is the same
eutils client with a third verb. It is a separate module rather than a branch inside that file
because it also speaks Datasets, and that file's own rule is that another API is a sibling.

#### The API key must never reach a message

`requests` puts the whole URL, query string included, into its exception strings, and `verify`
renders those into `DatabaseSequenceLink.detail` -- a column, shown on `/mutations/reference`
and printed by `./mutint ncbi_accessions --list`. So a deployment whose DNS failed published
its own `MUTINT_NCBI_API_KEY`, and the operator's email beside it. `ncbi.redact` takes both out
of anything derived from a failed request, and **every path that turns a `requests` exception
into words goes through it**. It is a substitution rather than a check because the key is a
value this code was handed, not a shape it could recognise.

Worth stating plainly because the settings invite it: the key and the address are the
**operator's**, used for every user's request. There is no per-user key and no UI for one. An
NCBI API key is a rate-limit identifier rather than an access credential, so the exposure is a
shared quota and a throttle that lands on the operator -- but it is a deployment-wide identity
either way, and `docs/using/configuration.md` says so where an operator will read it.
