...
from mutint_sample.models import UncalledRegion
from mutint_sample.functional_change import (
    FUNCTIONAL_CHANGE_TYPE_LIST, functional_change_bucket,
)
from mutint_sample.views.common import MUTATION_TYPE_LIST
import collections
import logging
...
def uncalled_bases_per_sample(sample_ids):
    ...
    **Bounds are inclusive**, hence the `+ 1`: `_is_uncovered` in mutint-phylogeny asks
...
def get_sample_info_list(samples):
    ...
    **Keyed by name, and this used to be an eleven-member tuple the template indexed.**
    `mutint_metadata.get_sample_info_list` was converted away from exactly that shape after
...
def _count_in_sql(queryset):
    ...
    **Both halves group; neither matches substrings.** The functional-change half used to be one
    ...
    `mutint_sample.functional_change`.
