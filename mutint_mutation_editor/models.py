...
The unit of change is an `mutint_sample.MutationCall` -- one sample's call of one
mutation -- and **never an `mutint_sample.Mutation`**. That distinction is load-bearing. Mutation
primary keys are stored as bare integers, with no foreign key and no pruning, in
mutint-phylogeny's `branch_mutations` JSON and in every exported CSV's "Mut ID" column.
...
class MutationEdit(models.Model):
    ...
    sample = models.ForeignKey("mutint_sample.Sample", on_delete=models.CASCADE,
                               related_name="+")
    mutation = models.ForeignKey("mutint_sample.Mutation", on_delete=models.SET_NULL,
                                 null=True, blank=True, related_name="+")
    # Where a copy came from. Null for a delete, and for a restore that re-adds a row.
    source_sample = models.ForeignKey("mutint_sample.Sample",
