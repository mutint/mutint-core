...
class ChangeMutationTestCase(EditorTestCase):
    ...
    def test_the_page_offers_only_the_samples_carrying_it(self):
        ...
        `data-value` is what `mutintSelectList` reads a row's id off -- a list rendered without
    ...
    def test_the_mutation_keeps_its_primary_key(self):
        ...
        as bare integers in mutint-phylogeny's JSON and in every exported CSV, and nothing
        refreshes them -- mutint-phylogeny discards the JSON rather than correcting it, and an
