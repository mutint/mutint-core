...
class GridPageTestCase(EditorTestCase):
    ...
    def test_the_mutation_row_itself_survives_the_delete(self):
        ...
        primary key is stored as a bare integer in mutint-converge, mutint-phylogeny and every
        exported CSV."""
        ids = [call.id for call in
               MutationCall.objects.filter(mutation=self.mut_1)]
    ...
    def test_deleting_the_last_call_removes_the_row(self):
        ...
        mutint-converge, mutint-phylogeny and every exported CSV -- so a mutation whose last
