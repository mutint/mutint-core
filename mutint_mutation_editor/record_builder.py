...
def build_genome_diff(mutation_type, attributes):
    """The verbatim GenomeDiff record, as `supplemental_data["mutint_core"]["genome_diff"]`
    stores it.
...
def annotate(genome_diff, experiment):
    ...
    unannotated and `./mutint reannotate` fills it in once a reference arrives, exactly as for a
