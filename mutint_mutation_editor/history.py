...
MUTATION_KEY_FIELDS = (
    "start_position", "seq_id", "mutation_type", "feature_length",
    "sequence_change", "gene",
)

#: The join from a MutationCall up to its experiment. Spelled once here; it is the same
#: traversal `mutint_sample.util` and `mutint_filter.util` use.
...
def mutation_identity(mutation):
    ...
    `supplemental_data` holds the record `to_gd_line()` round-trips for `gdtools APPLY` and `annotation` is what the
    ...
    identity["supplemental_data"] = mutation.supplemental_data
...
def genome_diff_from(identity):
    ...
    The identity carries `supplemental_data` whole so a foreign key survives a restore; a
    caller asking "did the mutation change" wants core's record out of it.
    """
    container = identity.get("supplemental_data") or {}
...
def mutation_for_identity(experiment, identity):
    ...
    return Mutation.objects.get_or_create(
        ...
        defaults={
            "supplemental_data": identity.get("supplemental_data") or {},
...
def apply_mutation_edit(experiment, user, mutation, identity, note=""):
    ...
    key, in mutint-phylogeny's `branch_mutations` and in every exported CSV -- and nothing
    refreshes them. What mutint-phylogeny does on a mutation edit is throw its cached trees
    ...
    **before** the row moves, or both sides of the edit set would record the new identity and
    ...
    mutation.supplemental_data = identity.get("supplemental_data") or {}
...
def rebuild_after_edit(experiment):
    ...
    **Unnarrowed by name, and narrowed by scope.** Both halves matter and they are different
    ...
    Never narrowed with `only=`, unlike `mutint_experiment.samples.rebuild_after_structural_change`
    ...
    That used to have a second and sharper reason: mutint-fixation cached MutationCall
