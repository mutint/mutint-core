...
def sequence_digest_stream(lines):
    ...
    The streaming twin of `aledb_import.reference.sequence_digest`, so a genome is never held
...
def _params(**extra):
    ...
    email = getattr(settings, "ALEDB_NCBI_EMAIL", "")
    api_key = getattr(settings, "ALEDB_NCBI_API_KEY", "")
    if email:
        params["email"] = email
    if api_key:
...
def _timeout():
    return getattr(settings, "ALEDB_NCBI_TIMEOUT", 30)


def _max_bases():
    return getattr(settings, "ALEDB_NCBI_MAX_BASES", 50_000_000)
...
def verified_contig_names(experiment):
    ...
    except (ExperimentReference.DoesNotExist, AttributeError):
        return frozenset()
