...
The files themselves come from `aledb_sample.views.alignments`, which serves BAM/BAI and the
...
def _resolve(request):
    ...
    try:
        ...
        # column, so the chain is named the way `aledb_sample.util` names it.
...
# The extent rule and the buffer moved to `aledb_sample.locus` when the NCBI Sequence Viewer
