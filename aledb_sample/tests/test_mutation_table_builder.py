...
from aledb_sample.util import get_all_calls_filtered
...
class AmplificationsRemovedTestCase(TestCase):
    ...
    def test_the_header_and_every_row_have_the_same_number_of_columns(self):
        ...
        from aledb_sample.views.mutation_table_builder import (
    ...
    def test_a_cell_links_to_the_browser_when_the_sample_has_an_alignment(self):
        from aledb_sample.util import get_reseq_ordered_dict
        from aledb_sample.views.mutation_table_builder import _get_table_mutation_entry
    ...
    def test_an_empty_cell_never_contains_the_true_marker(self):
        ...
        from aledb_sample.views.mutation_table_builder import HTML_EMPTY_MUTATION_CELL
...
class ManuallyAddedMutationTestCase(TestCase):
    ...
    `aledb_mutation_editor` writes a call with `present=True` and no caller flags --
...
class GeneCellTestCase(TestCase):
    ...
    def _cell(self, count):
        from aledb_sample.views.mutation_table_builder import get_gene_table_entry
    ...
    def test_a_list_over_the_limit_is_counted_rather_than_rendered(self):
        """Reached only by rows written before the importer capped -- `aledb_sample.0012` moves
