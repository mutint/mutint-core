...
could have, and it is the aledb-needle component now. So every test here registers its own and
...
ways under `./mutint test` the first time it ran, because aledb-needle's panel is in it. The
...
from aledb_common.panel_registry import (
    get_overview_panels,
...
class PanelRegistryTestCase(TestCase):
    ...
    def setUp(self):
        self.app_config = apps.get_app_config("aledb_common")
...
class RenderedPanelTestCase(TestCase):
    ...
    def _mine(self, panels, *names):
        ...
        Whatever else is installed registers panels too -- aledb-needle is one -- and they
    ...
    def test_a_panel_sees_the_context_processors(self):
        """`request=` is passed to the renderer, so a panel gets the same `aledb_version`,
    ...
    def test_a_panel_that_raises_is_dropped_and_the_others_render(self):
        ...
        with self.assertLogs("aledb_common.panel_registry", level="ERROR"):
