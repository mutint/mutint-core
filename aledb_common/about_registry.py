...
Apps call register_about_section() from their AppConfig.ready(); aledb_about's page renders
...
it keeps aledb-core free of any compile-time knowledge of the plugins: a plugin's prose
appears on the About page only when the plugin is installed, and no assembled project has to
be told about it.

**The unit is the component, not the Django app.** aledb-core is fifteen `aledb_*` apps and
...
def register_about_section(app_config, name=None, version=None, template=None):
    ...
    name        heading, e.g. 'aledb-core'. Defaults to the component directory's name.
    ...
    template    template name whose content becomes the section's body, e.g.
                'about/sections/aledb_core.html'. Omit it for a heading alone.
    """
...
def get_about_sections():
    ...
    from aledb_common.util import get_revision
