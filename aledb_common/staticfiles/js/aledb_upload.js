/* Chunked upload to aledb-core's staging area.
 *
 ...
 *
 * Loaded from base.html beside aledb_crud.js, so a page gets it with nothing to include.
 *
 * Usage:
 *
 *     aledbUpload(entries, {
 ...
 * directory. `aledbCollectDropped(dataTransfer)` builds that list from a drop, following
...
(function () {
    ...
    function postChunk(uploadId, path, offset, blob, onProgress) {
        return new Promise(function (resolve, reject) {
            ...
            xhr.setRequestHeader("X-CSRFToken", window.aledbCsrfHeader());
            if (onProgress) {
    ...
    window.aledbUpload = function (entries, options) {
        options = options || {};
        ...
        var uploadId = null;
        return window.aledbPostJson(url, body).then(function (session) {
     ...
     */
    window.aledbCollectDropped = function (dataTransfer) {
        ...
        return Promise.resolve(window.aledbFromFileList(dataTransfer.files));
    };

    /* `[{file, path}]` from an <input type=file>, honouring webkitRelativePath. */
    window.aledbFromFileList = function (fileList) {
