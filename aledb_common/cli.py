...
def manage():
    ...
    # be overridden the usual way. `./aledb version` should report aledb-core, so
    # dispatch that one command directly. It is still listed under aledb_common in
    # `./aledb help`, and still reachable as a normal command elsewhere.
    if sys.argv[1:2] == ['version']:
        import django
        django.setup()
        from aledb_common.management.commands.version import Command
