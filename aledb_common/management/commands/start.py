...
#: Short deliberately -- see aledb_jobs/supervisor.py's GRACE_SECONDS, which owns the reason.
...
def _entry_script():
    """The `./aledb` (or `./mutint`) script that invoked us, absolutely."""
...
def _spawn_worker():
    ...
    **The whole point is that the queue actually drains.** Before this, `./aledb start` printed
    ...
    # is what makes the next `./aledb start` fail with "That port is already in use". That has
    ...
    # job control -- any interactive shell -- the shell puts `./aledb start` in a group of its
    ...
    # recorded. An unowned cluster -- `./aledb db start`, which exists so a server outlives one
...
class Command(BaseCommand):
        ...
        if is_first_launch():
            ...
            if worker is not None:
                ...
                self.stdout.write(
                    ...
                    '  `./aledb db_worker` in another terminal, or `./aledb coverage`.\n')
    ...
    def _start_worker(self, no_worker):
        ...
        if no_worker:
            return None
        # Asked through aledb_jobs, which is already the one place in aledb-core that knows
        ...
        # stop the worker. See aledb_jobs/supervisor.py.
