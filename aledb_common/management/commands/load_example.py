"""Load a component's example dataset.
    ...
    ./aledb load_example                            # what is available
    ./aledb load_example aledb-fixation-example     # load it
    ./aledb load_example aledb-fixation-example --replace

A component that needs particular data to show its feature ships that data and registers it
(``aledb_common.example_registry``); this loads it. The dataset is a directory laid out as it
...
from aledb_common.example_registry import get_example_dataset, get_example_datasets
...
class Command(BaseCommand):
    ...
    def _load(self, dataset, options):
        ...
        staged_root = tempfile.mkdtemp(prefix="aledb-example-")
        try:
