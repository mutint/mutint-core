...
ALEdb is PostgreSQL-only. That would ordinarily cost the property aledb-core is built
...
``aledb_common.tools.tool_path()``", and no application code has any business calling
...
be tested (``aledb_common/tests/test_pg.py``) without starting anything.
...
``ALEDB_DB_HOST`` decides, and one variable rather than two near-synonyms: **unset means
...
itself for the owner. ``aledb db start`` deliberately starts an *unowned* cluster, which is
...
def is_external():
    ...
    `ALEDB_DB_MANAGED` is what distinguishes somebody else's host from the one we just
    ...
    """
    return (bool(os.environ.get('ALEDB_DB_HOST'))
            and os.environ.get('ALEDB_DB_MANAGED') != '1')


def database_name(base_dir):
    """The database name, derived from the checkout's directory name.

    Derived rather than configured, so `aledb-core`, `mutint` and `aledb-deploy` differ by
    ...
    settings_local.py, which is one of the places an aledb-deploy merge used to conflict every
    time.
    """
...
class Cluster(object):
    ...
    def _make_socket_dir(self):
        ...
        if os.stat(path).st_uid != os.getuid():
            raise PostgresError(
                "%s is not owned by you. Remove it, or set ALEDB_DB_HOST to point at a "
    ...
    def env(self):
        ...
        return {
            'ALEDB_DB_HOST': self.socket_dir,
            'ALEDB_DB_PORT': PORT,
            'ALEDB_DB_NAME': self.name,
            'ALEDB_DB_USER': USER,
            'ALEDB_DB_MANAGED': '1',
        }
    ...
    def describe(self):
        ...
        if is_external():
            lines.append("mode:      external (ALEDB_DB_HOST is set; nothing is managed here)")
            lines.append("host:      %s" % os.environ['ALEDB_DB_HOST'])
            return lines
