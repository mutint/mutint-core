...
(`aledb_common.util._git`, the browser-opening `open` in the start command) both use a bare
...
Where to look is ``ALEDB_TOOLS_DIR``, which the entry script exports before it re-execs. It is
not derived from settings on purpose: an assembled project reaches ``get_base_settings()``
through aledb-core's ``config/defaults.py``, which passes the *aledb-core* directory as
...
def tools_dir():
    ...
    return getattr(settings, "ALEDB_TOOLS_DIR", None)
...
def require(name):
    ...
    raise ToolMissing(
        ...
        "`./aledb install` (or `./mutint install`) to build env/tools, or put %s on PATH."
        % (name, name))
