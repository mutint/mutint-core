"""The deadman that stops the worker, and the cluster, when `./aledb start` dies.
...
from aledb_jobs import supervisor
...
class ConstraintsTestCase(TestCase):
    ...
    def test_it_imports_nothing_from_django(self):
        ...
        Under an assembled project `aledb_jobs` is importable only because `config/settings.py`
        ...
        moment; it would fail every time a worker was started, under MutInt and aledb-deploy
        and not under standalone aledb-core, which is the worst way to find out.
        """
    ...
    def test_it_finds_pg_by_path(self):
        """`_stop_cluster` walks up from this file to `aledb_common/pg.py`. A move of either
...
class StopTestCase(unittest.TestCase):
    ...
    what `./aledb reap_jobs` clears up after.
    """
...
class ClusterHalfTestCase(unittest.TestCase):
    ...
    def test_it_does_nothing_when_it_was_told_nothing(self):
        ...
        `./aledb db start`, which exists so a server outlives one command -- must be left
        running."""
...
class StopGroupTestCase(unittest.TestCase):
    ...
    `restart_with_reloader` spawns runserver as an ordinary child of `./aledb start`, so a
    `kill -9` there orphans it still holding port 8000 -- which is what makes the next
    `./aledb start` fail with "That port is already in use". Pre-existing, and not something a
...
class OptionsTestCase(unittest.TestCase):
    ...
    def test_they_are_pulled_out_and_the_positionals_survive(self):
        argv = ["--parent", "12", "7", "--group", "34", "./aledb", "--pg-dir", "/x"]
        ...
        self.assertEqual(["7", "./aledb"], argv)

    def test_an_absent_one_takes_the_default(self):
        self.assertEqual(0, supervisor._option(["7", "./aledb"], "--group", 0))
