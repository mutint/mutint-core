...
from mutint_common.util import GENE_LIST_LIMIT, get_gene_list
from mutint_sample.util import get_ecocyc_gene_list
from django.utils.safestring import mark_safe

blank_field = {"blank": True, "null": True}


# TODO: Refactor: figure out how to get a Sample to return its list of mutation calls and remove functionality from mutint_sample.views.common
class SupplementalDataMixin(models.Model):
    ...
    the finding, but everything shipped alongside so somebody can check it::
        ...
        {"mutint_core": {"genome_diff": { ...the verbatim breseq record... }}}
        {"mutint_core": {"breseq": {...}, "sequencing": {...}, "curation": {...}}}

    **Two levels, and the outer one is a component rather than a Django app label** -- core
    is fifteen apps, so `mutint_core` names the checkout, the same unit `about_registry` keys
    ...
    **What belongs here**: something that arrives with an import, shares the row's lifetime,
    ...
    Shaped like `SoftDeleteMixin` in `mutint_experiment.models` -- an abstract model carrying
...
class Sample(SupplementalDataMixin):
    ...
    **Three models used to be here** -- `Isolate` held where the sample sat and what it was
    ...
    #: what mutint-fixation orders by to take a population's last two, and filtering on it is
    ...
    #: its time point by convention rather than by constraint; `mutint_experiment.samples`
    ...
    # not a path: the location is derived from this row's pk by mutint_common.store. Three
    ...
    def curation(self):
        ...
        is why `mutint_experiment.samples._MAX_LENGTHS` still refuses an over-long value. The
    ...
    def label(self):
        ...
        no longer exist and the third had moved. `mutint_experiment.coordinates` owns the
...
class UncalledRegion(models.Model):
    ...
    **It is not only a statistic.** mutint-phylogeny reads these to decide which cells of its
    ...
    changes the parsimony score with nothing raised. `mutint_stats` counts them per sample and
    `mutint_import.reference_rename` rewrites `seq_id` with everything else that stores one.
...
class Mutation(SupplementalDataMixin):
    ...
    # ---- breseq annotation, generated at import by mutint_import.annotate -----
    #
    ...
    # mutint_dashboard.util substring-matching the rendered protein_change.
    ...
    #: what `annotate.annotator.mutation_interval` computed, and for everything MutInt stores
...
class MutationCall(SupplementalDataMixin):
    ...
    **It carries `supplemental_data` as well as `evidence`, and the two are not duplicates.**
    ...
    `mutint_import/vcf_import.py` for why a shared `Mutation` is the wrong home for any of it.
    """
    ...
    # Being always null was not merely useless: `mutint_filter` ANDed a `frequency_gatk__lt`
...
class ReferenceSequences(models.Model):
    ...
    `experiment_id` -- see mutint_common.store, whose `experiment_reference_dir` keeps that
    ...
    **Plural because one row holds many sequences**: `seq_ids` is the contig list, so this
    ...
    experiment = models.OneToOneField("mutint_experiment.Experiment",
...
class DatabaseSequenceLink(models.Model):
    ...
    **`database` is why this is not called `NcbiSequence`, which it was.** Nothing above is
    ...
    `mutint_sample.ncbi` and its templates rather than here.
    ...
    **It is a discriminator, not a plugin seam.** There is exactly one value and exactly one
    ...
    #: mutint_import.reference.sequence_digest() of this contig -- sha256 of its uppercased
    ...
    #: Why it failed, in words, for a page and for `./mutint ncbi_accessions --list`. A status
