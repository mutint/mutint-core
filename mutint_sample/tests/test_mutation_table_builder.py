...
class AmplificationsRemovedTestCase(TestCase):
    ...
    def test_the_header_and_every_row_have_the_same_number_of_columns(self):
        ...
        from mutint_sample.views.mutation_table_builder import (
        ...
        body = get_mutation_table_body(self.user, call, reseq_dict, self.experiment)
    ...
    def test_the_close_icon_column_is_gone(self):
        ...
        delete-that-is-not-a-delete `mutint_mutation_editor` replaces."""
        from mutint_sample.util import get_reseq_ordered_dict
        from mutint_sample.views.mutation_table_builder import get_mutation_table_body
    ...
    def test_a_cell_links_to_the_browser_when_the_sample_has_an_alignment(self):
        from mutint_sample.util import get_reseq_ordered_dict
        from mutint_sample.views.mutation_table_builder import _get_table_mutation_entry

        reseq_dict = get_reseq_ordered_dict(self.experiment.id, None, None, None)
        call = sorted(get_all_calls_filtered(self.experiment.id),
                          key=lambda o: o.id)[0]
        reseq_dict[call.sample_id].bam_stored = True
    ...
    def test_a_cell_is_plain_text_when_the_sample_has_no_alignment(self):
        ...
        call = sorted(get_all_calls_filtered(self.experiment.id),
                          key=lambda o: o.id)[0]
        reseq_dict[call.sample_id].bam_stored = False
    ...
    def test_an_empty_cell_never_contains_the_true_marker(self):
        ...
        from mutint_sample.views.mutation_table_builder import HTML_EMPTY_MUTATION_CELL
...
class ManuallyAddedMutationTestCase(TestCase):
    ...
    def _add_by_hand(self):
        ...
        `build_call` rather than a literal dict on purpose: this test is about what
...
class GeneCellTestCase(TestCase):
    ...
    def _cell(self, count):
        from mutint_sample.views.mutation_table_builder import get_gene_table_entry

        # `Mutation.gene` is comma-SPACE separated -- that is what
        # `mutint_common.util.GENE_RANGE_ANNOTATION_DELIMITER` splits on, and a bare comma
    ...
    def test_a_list_over_the_limit_is_counted_rather_than_rendered(self):
        """Reached only by rows written before the importer capped -- `mutint_sample.0012` moves
