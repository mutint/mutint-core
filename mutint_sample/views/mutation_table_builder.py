...
from mutint_common.constants import TAGS, ROW_TAGS, COLUMN_TAGS, HTML_MUTATION_TABLE_HEADER
from mutint_experiment.models import Experiment
from mutint_experiment.permissions import can_curate
...
def _build_table_cell_for_dropdown(mutation, experiment):
    ...
    is `mutint_mutation_editor`'s job now, so what is left here is tagging.
    """
...
def get_gene_table_entry(mutation):
    ...
    breseq-style table, which is different markup from `mutint_import.annotate.display` and has
    ...
    **The wrapper is closed once, here, rather than inside each branch.** It went unclosed on
    ...
    # written before it existed -- `mutint_sample.0012` moves the ones this database had. Counted
...
def _refseq_cell(mutation, verified_contigs):
    ...
    **Every contig is linked, verified or not**, and that is a correction rather than the
    ...
    `REFSEQ_COLUMN_IN_MUT_TABLE`, and `mutint_export.util` re-derives this value itself rather
