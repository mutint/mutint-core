...
`mutint_mutation_editor`'s job now -- per sample, recorded, and restorable.

`@ajax` puts the real status in the JSON body and always sends HTTP 200 (see
`mutint_common.ajax`), so a refusal returns HttpResponseForbidden and the caller reads
...
from mutint_common.ajax import ajax

from mutint_experiment import permissions
...
def _toggle(existing, selected_tag):
    ...
    dict in `mutint_common.constants`, which contains none.
    """
    if not existing:
        return selected_tag
