...
`Mutation.snp_type` is breseq's own functional class, written by `mutint_import.annotate.annotator`
(a port of `cReferenceSequences::annotate_1_mutation`) and promoted to an indexed column by
`mutint_import.annotation.PROMOTED_COLUMNS`. This module turns one of its values into one of the
...
added to `mutint_import.annotation.PROMOTED_COLUMNS` with a backfill, not a fallback here.
"""
