"""Record and verify the NCBI accession of a reference contig.
    ...
    ./mutint ncbi_accessions --list                     # what is recorded; touches no network
    ./mutint ncbi_accessions 4 --list                   # just this experiment's contigs
    ./mutint ncbi_accessions 4 --seq-id NC_000913 \
                              --accession NC_000913.3  # state one, and check it
    ./mutint ncbi_accessions --recheck                  # re-verify everything recorded
    ./mutint ncbi_accessions 4 --recheck                # ... for one experiment

**Nothing is inferred.** A bare run has nothing to propose, because an accession is only ever
stated by a person -- see `mutint_sample.ncbi` for why guessing from a contig name is refused.
...
class Command(BaseCommand):
    ...
    def _contigs(self, experiment_id):
        ...
        Read from `ReferenceSequences.seq_ids`, which already carries the per-contig digest
        the whole check compares against -- no reference file is opened here or anywhere.
        """
        references = ReferenceSequences.objects.select_related("experiment")
