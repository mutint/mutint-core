...
dataset, which is `aledb_mutation_editor`'s job and is logged and reversible, with choosing what
...
SESSION_KEY = "aledb_view_filters"
...
def get_view_filter(request, experiment_id):
    ...
    **Presence, not truthiness**, which is what makes clearing work: the clear link sends the
    ...
    to clear.
    """
    cache = getattr(request, "_aledb_view_filter_cache", None)
    if cache is None:
        cache = request._aledb_view_filter_cache = {}
