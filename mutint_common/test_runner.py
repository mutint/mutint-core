...
`./mutint test` works by ordinary unittest discovery: mutint-core's app packages sit directly
...
project's code lives in submodule directories named `mutint-core`, `mutint-compare` and so on.
...
The runner also points `MUTINT_STORE_DIR` at a temporary directory for the duration of the
run. Tests that exercise the importer write real files into the store, and one that forgets
`override_settings(MUTINT_STORE_DIR=...)` writes them into the *live* store instead. That is
...
class MutintTestRunner(DiscoverRunner):
    ...
    #: back (mutint_import/tests/test_tasks.py).
    ...
    def setup_test_environment(self, **kwargs):
        ...
        self._store_dir = tempfile.mkdtemp(prefix="mutint-test-store-")
        self._store_override = override_settings(MUTINT_STORE_DIR=self._store_dir)
