 ...
 * mutintConfirmDelete is a plain yes/no, for revoking a grant or a group membership;
 * mutintConfirmTypedDelete makes the person type DELETE, for the four controls that
 * destroy data.
 *
...
(function () {
     ...
     * every sample, mutation and alignment ever imported into it. mutintConfirmDelete above
     ...
     * `./mutint delete` does not honour and one curl away from bypass anyway.
     */
    window.mutintConfirmTypedDelete = function (what) {
        return swal({
            title: "Delete " + what + "?",
    ...
    window.mutintDeleteSelected = function (opts) {
        ...
        window.mutintConfirmTypedDelete(ids.length + " " + opts.noun + "(s)")
