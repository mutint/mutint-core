...
Extracted from ``mutint_sample.views.alignments`` when the legacy ``/aledata/`` route shared it.
...
def serve_file(request, path, filename, content_type=None, headers=None):
    ...
    header is the whole of the failure. `mutint_sample/views/report.py` is why it exists: it
