...
Some things in MutInt are too expensive to compute per request. `mutint_dashboard`'s count
...
sites inside `mutint_import`.

**The list is much shorter than it was, and the direction is worth knowing before adding to
it.** `mutint_stats.StaticData` (the needle plot), `mutint_stats.ExperimentSummary` (the
Overview's counts), `mutint_converge.ConvergeMutation` and `mutint_fixation.FixatedMutation` were
...
*correct* because the dashboard applies no filter -- see `mutint_dashboard.util`. Anything the
...
experiment and so have to come last -- but `mutint_dashboard` is the second app in the
`mutint_*` block, because that is where its *nav entry* belongs. INSTALLED_APPS order cannot
...
takes and for the same reason: derived data that is stale is recoverable -- `./mutint rebuild`
...
`./mutint rebuild --list` is where it shows up, and it is why `last_error` is a column rather
...
#
# What it bought was worth having at the time: `mutint_phylogeny` reads MutationCall directly
...
def register_rebuilder(name, fn, scope=EXPERIMENT_SCOPE, label=None,
    ...
    way it does not for nav entries: `mutint_stats`'s summary is computed from the mutations, so
    anything that *changes* the mutations has to be registered ahead of it.

    name   stable identifier, e.g. 'fixation'. It is what `only=` and `./mutint rebuild --only`
    ...
    label  human-readable name for `./mutint rebuild --list`; defaults to name.
    ...
    auto   False registers derived data that is *tracked* but never rebuilt behind anyone's
           ...
           stale without promising to recompute it -- the thing `mutint-phylogeny` wanted and
...
def get_rebuilders(scope=None, only=None):
    ...
    only   an iterable of names to restrict to. **An unknown name is skipped, not an error.**
           ...
           raise where it would simply have nothing to do. `./mutint rebuild --only` checks the
...
def run_rebuilds(experiment_id=None, only=None, force=False, scope=None):
    ...
    for rebuilder in get_rebuilders(scope=scope, only=only):
        ...
        if not rebuilder['auto'] and (named is None or name not in named):
            ...
            # which is what `./mutint rebuild --only mutint_phylogeny` is. `force` is checked
        ...
        if target is None and not site_scoped:
            # An experiment-scoped rebuild with no experiment named. `./mutint rebuild --all`
