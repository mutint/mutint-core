...
`./mutint docs` in mutint-core builds mutint-core's documentation. The same command in an
...
they want to use MutInt or extend it, and rarely wants "everything mutint-fixation has to say".
...
def project_root():
    ...
    `MUTINT_TOOLS_DIR` is `<project root>/env/tools`, exported by `./mutint` or `./mutint` before
    it re-execs. It is the only thing that knows: settings cannot, because an assembled project
    reaches `get_base_settings()` through mutint-core's `config/defaults.py`, which passes the
    *mutint-core* directory -- the same trap `templates/` and `staticfiles/` work around.
    """
...
def contributing_components(root):
    ...
    **The project itself is excluded**, even though its own apps are installed. Standalone,
    mutint-core *is* the project, and without this it would be collected as a component of
...
def build_config(project_root_dir, project_config, components):
    ...
    # Every contributing component, absolutely, or a plugin's `::: mutint_yourthing.util` does
