...
`MutintTestRunner` redirects `MUTINT_STORE_DIR` at a temporary directory for the whole run,
because a test that exercises the importer and forgets its own
`override_settings(MUTINT_STORE_DIR=...)` otherwise writes into the live store -- and store
