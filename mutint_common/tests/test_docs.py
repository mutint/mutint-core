...
the toolchain lives in `requirements-docs.txt`, which only `./mutint docs` installs -- so a test
...
COMMON_DIR = os.path.join(BASE_DIR, "mutint_common")
    ...
    def test_every_reference_page_points_at_its_module(self):
        ...
        for module in registry_modules():
            with self.subTest(module=module):
                ...
                self.assertIn("::: mutint_common.%s" % module, text)
...
class ProjectRootTestCase(SimpleTestCase):
    ...
    `./mutint docs` is inherited by every assembled project -- both entry scripts end at
    `mutint_common.cli.manage()` -- so `./mutint docs` reaches the same command. It used to
    build the *submodule's* docs from there, because its base directory came from `__file__`.
    Now it builds the project's manual, and `MUTINT_TOOLS_DIR` is what tells it which project
    ...
    mutint-core's `config/defaults.py`.
    """

    def with_tools_dir(self, value):
        from mutint_common import docs_manual

        original = os.environ.get("MUTINT_TOOLS_DIR")
        if value is None:
            os.environ.pop("MUTINT_TOOLS_DIR", None)
        else:
            os.environ["MUTINT_TOOLS_DIR"] = value
    ...
    def _restore(self, original):
        if original is None:
            ...
            os.environ["MUTINT_TOOLS_DIR"] = original
    ...
    def test_no_entry_script_means_no_answer(self):
        ...
        compare against. The command falls back to mutint-core rather than guessing."""
    ...
    def components_for(self, dirs, root):
        ...
        import mutint_common.about_registry as about
    ...
    def test_the_project_is_not_a_component_of_itself(self):
        """Standalone, mutint-core *is* the project and its apps are installed. Without this
    ...
    def test_a_component_without_a_config_is_skipped(self):
        ...
        found = self.components_for(["/nonexistent/mutint-yourthing"], "/some/project")
...
class ManualAssemblyTestCase(SimpleTestCase):
    ...
    def test_audiences_merge_across_components(self):
        ...
        mutint-fixation has to say"."""
        from mutint_common.docs_manual import EXTENDING, USING

        nav = self.merge(
            {"nav": [{USING: [{"Quick start": "using/quickstart.md"}]},
                     {EXTENDING: [{"Writing a plugin": "plugin/index.md"}]}]},
            {"mutint-fixation": {"nav": [{USING: [{"Fixed mutations": "using/fix.md"}]}]}})

        self.assertEqual([{"Quick start": "using/quickstart.md"},
                          {"Fixed mutations": "mutint-fixation/using/fix.md"}],
    ...
    def test_a_components_paths_are_prefixed_and_the_projects_are_not(self):
        ...
        from mutint_common.docs_manual import USING

        nav = self.merge(
            {"nav": [{USING: ["a.md"]}]},
            {"mutint-yourthing": {"nav": [{USING: ["b.md"]}]}})

        self.assertEqual(["a.md", "mutint-yourthing/b.md"], self.section(nav, USING))
    ...
    def test_nested_sections_are_prefixed_all_the_way_down(self):
        from mutint_common.docs_manual import EXTENDING
    ...
    def test_an_unrecognized_heading_is_filed_not_dropped(self):
        ...
        from mutint_common.docs_manual import DEPLOYMENT
    ...
    def test_the_sections_are_always_in_the_same_order(self):
        from mutint_common.docs_manual import AUDIENCES, DEPLOYMENT, EXTENDING, USING
...
class GeneratedConfigTestCase(SimpleTestCase):
    ...
    def test_every_component_is_on_the_mkdocstrings_path(self):
        """Or a plugin's `::: mutint_yourthing.util` does not resolve once its pages are being
        built from somewhere else."""
        config = self.config([("mutint-core", "/c"), ("mutint-fixation", "/f")])
    ...
    def test_component_sources_are_watched(self):
        ...
        config = self.config([("mutint-fixation", "/f")])
