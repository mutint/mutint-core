...
def register_post_experiment_hook(fn):
    ...
    `mutint_common.rebuild_registry.register_rebuilder`, which is the general form: a
    ...
    The rebuilder is named after the app the function came from -- `mutint_fixation`,
    `mutint_converge` -- which is what `./mutint rebuild --only` and the `only=` argument take.
    ...
    """
    from mutint_common.rebuild_registry import get_rebuilder, register_rebuilder
...
def register_sequence_rename_hook(fn):
    ...
    *derived* from those names is core's blind spot, and a plugin's derived data keyed on
    ...
    mutint-phylogeny is the live example: its character-matrix columns are identified by
