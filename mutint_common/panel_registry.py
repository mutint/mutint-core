...
`register_plugin_urlpatterns` plus `register_nav_item`, which is what mutint-compare,
mutint-fixation and mutint-converge are. Something that is one panel and not a page had no seam
at all, so it had to live in core: the needle plot sat in `mutint_stats` beside the Overview's
...
def render_overview_panels(experiment, request):
    ...
    the same `mutint_version`, user and static configuration the page around it does.
    """
