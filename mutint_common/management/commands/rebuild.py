"""Recompute derived data -- core's and every plugin's -- from the command line.
    ...
    ./mutint rebuild --list          what is registered, and what is currently stale
    ./mutint rebuild 4               everything stale for experiment 4
    ./mutint rebuild 4 --force       everything for experiment 4, stale or not
    ./mutint rebuild --all           every live experiment, plus the site-wide totals
    ./mutint rebuild --all --only overview --only mutint_fixation
    ...
    def handle(self, *args, **options):
        ...
        if totals["failed"]:
            self.stdout.write(self.style.WARNING(
                "failures are recorded, not raised -- see ./mutint rebuild --list"))
    ...
    def _show_list(self):
        ...
        for rebuilder in rebuilders:
            ...
            if not rebuilder["auto"]:
                ...
                # a bare `./mutint rebuild 4` leaving it stale is the design rather than a
