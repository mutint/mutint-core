...
CODING_CHANGE_TYPE_LIST = ['intergenic', 'noncoding', 'pseudogene', 'snp_type_synonymous', 'snp_type_nonsynonymous', 'Duplication', 'Unannotated']
...
SEQ_COLORS = _set_colors(len(CODING_CHANGE_TYPE_LIST) - 1)
...
def set_sequence_change_bar_chart_colors(sequence_changes):
    for seq_change in sequence_changes:
        has_match = False
        for protein in CODING_CHANGE_TYPE_LIST:
            if protein in seq_change['mutation__protein_change']:
                seq_change['color'] = COLORS[CODING_CHANGE_TYPE_LIST.index(protein)]
                has_match = True
                break
