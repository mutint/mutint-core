...
class GDParser():
        ...
        if ver_line[:13] != '#=GENOME_DIFF':
            ...
            for line_num, line in enumerate(file_handle.readlines()):
                try:
                    ...
                    print "Parse error in field {}:{}, could not parse {}:".format(gdfe.field_num, gdfe.field_name,
                                                                                   gdfe.field_value)
        ...
        for mut_id in self.data['mutation']:
            for ev in self.data['mutation'][mut_id]['parent_ids']:
                if ev not in self.data['evidence'] and ev != 'manual':
                    raise GDFieldError(3, 'parent_ids', item['parent_ids'],
                                       "Error on line {}, invalid parent id: {}".format(self.id2line_num[item], ev))
        ...
        for field_idx, field in enumerate(source_data[start_field:start_field + len(field_defs)]):
            ...
            if field_defs[field_idx]['type'] == 'string':
                ...
                if 'max_length' in field_defs[field_idx] and len(parsed_field) > field_defs[field_idx]['max_length']:
                    raise GDFieldError(field_idx + start_field, field_defs[field_idx]['name'], field,
                                       "Length of string field {} exceeds max length of {}".format(len(parsed_field),
                                                                                                   field_defs[
                                                                                                       field_idx][
                                                                                                       'max_length']))
    ...
    def _parseLine(self, line_num, line):
        ...
        if line[:2] == '#=':
            ...
            line_elements = re.split('\s', line[2:],
                                     1)  # Variable name and value are delineated by the first whitespace character
                ...
                if item_class == 'mutation' or item_class == 'evidence':
                    if data_elements[2] == '.':
                        ...
                        try:
                            ...
                            raise GDFieldError(2, 'parent_ids', data_elements[2],
                    ...
                    raise GDFieldError(3, 'parent_ids', data_elements[2],
