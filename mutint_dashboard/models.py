...
class InstallationCounts(models.Model):
    ...
    **This was three tables** -- `InventoryCounts` and a pair, `MutationCallCounts` and
    ...
    `mutint_stats.util._empty_counts` builds the same dicts for the per-experiment Overview and
    ...
    #: them are invalidated by different things. `mutint_experiment.samples` narrows to
